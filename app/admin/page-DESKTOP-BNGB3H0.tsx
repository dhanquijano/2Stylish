    ...
    return (
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Appointments Dashboard</h1>
            <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2">Manage your appointments and schedule.</p>
          </div>
                ...
                {appointments
                  ...
                  .map((appointment) => (
                    <TableRow key={appointment.id}>
                      <TableCell className="text-xs sm:text-sm">{appointment.appointmentTime}</TableCell>
                      <TableCell className="text-xs sm:text-sm">{appointment.fullName}</TableCell>
                      <TableCell className="max-w-[200px] truncate text-xs sm:text-sm hidden sm:table-cell">{appointment.services}</TableCell>
                      <TableCell className="text-xs sm:text-sm hidden md:table-cell">{appointment.barber || 'No Preference'}</TableCell>
                      <TableCell>
                        {getStatusBadge(
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2">Welcome back! Here's what's happening with your business today.</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <Button variant="outline" onClick={fetchDashboardData} className="flex-1 sm:flex-none">
            <Activity className="h-4 w-4 mr-2" />
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  <Link href="/admin/appointments">
                    <Button variant="outline" className="w-full h-16 sm:h-20 flex flex-col gap-1 sm:gap-2">
                  ...
                  <Link href="/admin/scheduling">
                    <Button variant="outline" className="w-full h-16 sm:h-20 flex flex-col gap-1 sm:gap-2">
                      ...
                      <span className="text-xs sm:text-sm">Staff Schedule</span>
                    </Button>
                  </Link>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-xs sm:text-sm hidden sm:table-cell">Services</TableHead>
                      <TableHead className="text-xs sm:text-sm hidden md:table-cell">Barber</TableHead>
                ...
                {barbers.slice(0, 6).map((barber) => (
                  <div key={barber.id} className="p-3 sm:p-4 border rounded-lg">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-primary-admin rounded-full flex items-center justify-center flex-shrink-0">
                        <span className="text-white font-semibold text-sm sm:text-base">
                          {barber.name?.charAt(0) || 'U'}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-sm sm:text-base truncate">{barber.name}</p>
                        <p className="text-xs sm:text-sm text-muted-foreground truncate">{barber.branches?.join(', ') || 'No Branch'}</p>
                      </div>
                    </div>
                  </div>
                ))}
