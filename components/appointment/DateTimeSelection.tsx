      ...
      {availableDates.length === 0 ? (
        ...
        <FormField
          ...
          render={({ field }: any) => {
            ...
            console.log("Available dates for calendar:", availableDates);
            const saturdays = availableDates.filter(date => {
              const d = new Date(date);
              return d.getDay() === 6;
            });
            console.log("Saturdays in available dates:", saturdays);
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto">
                  {timeSlots.map((slot) => (
                    <Button
                      ...
                      className={`h-10 sm:h-12 text-xs sm:text-sm ${slot.available
                          ? field.value === slot.time
                    ...
                    >
                      ...
                      {slot.available &&
                        field.value === slot.time && (
                          <Badge
                            variant="secondary"
                            className="ml-1 text-xs bg-white text-blue-600 hidden sm:inline-flex"
                          >
                            Selected
                          </Badge>
                        )}
