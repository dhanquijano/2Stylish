...
export const getAvailableTimeSlots = async (
  ...
  try {
    ...
    console.log(`[getAvailableTimeSlots] Checking availability for:`, {
      date,
      barberId,
      barberName,
      branchId,
    ...
    const timeSlots: TimeSlot[] = allTimeSlots.map((time) => {
      ...
      const inShift = withinAnyShift(time);
      const notLeave = notOnLeave(time);
      const available = !isBooked && inShift && notLeave;
      
      if (isBooked) {
        console.log(`[getAvailableTimeSlots] Time ${time} is BOOKED`);
      }
    ...
    console.log(`[getAvailableTimeSlots] Returning ${timeSlots.filter(s => !s.available).length} unavailable slots out of ${timeSlots.length} total`);
