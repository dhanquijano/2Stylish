...
export const sales = pgTable("sales", {
  ...
  isManual: integer("is_manual").notNull().default(0), // 0 = false, 1 = true
  notes: text("notes"),
  receiptUrl: text("receipt_url"),
  appointmentType: text("appointment_type").default("reservation"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow(),
}, (table) => ({
  dateIdx: index("sales_date_idx").on(table.date),
  branchIdx: index("sales_branch_idx").on(table.branch),
